package cn.cienet.electriccalculator.chart.view;
...
import cn.cienet.electriccalculator.chart.anim.AlphaAnim;
import cn.cienet.electriccalculator.chart.anim.Anim;
import cn.cienet.electriccalculator.chart.anim.TranslateAnim;
...
public class Chart extends View{
    ...
    private void initChartProp() {
        ...
        //y轴宽度
        yHeight = getHeight() - getPaddingTop() - getPaddingBottom() - xTextSurplus - titleTextSize;
        //x轴每个刻度的间距
    ...
    private void initCoordinateSystem(Canvas canvas){
        ...
        mainPaint.setTextSize(titleTextSize);
        //draw x 坐标
        ...
        if (chartTitle!=null) {
            canvas.drawText(chartTitle,
            		xWidth/2-chartTitle.length()*titleTextSize/3,
            		yHeight+titleTextSize*3,
            		mainPaint);
		}
        //刻度值画笔
        ...
        for (int i=0; i<xCoordinates.length; i++){
            ...
            if (xData.length<6) {
				...
				drawText(canvas, xData[i], oX+xCoordinates[i]-textX/4, oY+textY, textPaint, -65);
			}         
        }
        if(yMax == 0f){
            yMax = getYMax();
